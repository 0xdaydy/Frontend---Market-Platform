...
class FarmerRemoteDataSource {
      ...
      try {
        final list = switch (response.data) {
          ...
          final Map<String, dynamic> m =>
            m.jsonList('data', fallbackKeys: const ['farmers']),
        ...
        AppLogger.error(
            'FarmerRemoteDataSource: Failed to parse getFarmers: $e', [e, st]);
  ...
  FutureEither<FarmerModel> getFarmer(int id) async {
    ...
    return result.flatMap((response) {
      try {
        ...
        AppLogger.error(
            'FarmerRemoteDataSource: Failed to parse getFarmer: $e', [e, st]);
  ...
  FutureEither<FarmerModel> createFarmer(Map<String, dynamic> data) async {
    ...
    return result.flatMap((response) {
      try {
        ...
        final farmer =
            FarmerModel.fromJson(responseData['data'] ?? responseData);
        return right(farmer);
      } catch (e, st) {
        AppLogger.error(
            'FarmerRemoteDataSource: Failed to parse createFarmer: $e',
            [e, st]);
        return left(
            ServerFailure('Failed to parse created farmer: $e', error: e));
      }
    });
  }
    ...
    return result.flatMap((response) {
      try {
        final list = switch (response.data) {
          final List<dynamic> l => l,
          final Map<String, dynamic> m =>
            m.jsonList('data', fallbackKeys: const ['debts']),
          _ => throw const FormatException('Expected Map or List response'),
        };
        final debts = list
        ...
        AppLogger.error(
            'FarmerRemoteDataSource: Failed to parse getFarmerDebts: $e',
            [e, st]);
  ...
  FutureEither<List<TransactionModel>> getFarmerTransactions(
      int farmerId) async {
    ...
    return result.flatMap((response) {
      try {
        final list = switch (response.data) {
          final List<dynamic> l => l,
          final Map<String, dynamic> m =>
            m.jsonList('data', fallbackKeys: const ['transactions']),
          _ => throw const FormatException('Expected Map or List response'),
        };
        ...
        AppLogger.error(
            'FarmerRemoteDataSource: Failed to parse getFarmerTransactions: $e',
            [e, st]);
        return left(
            ServerFailure('Failed to parse transactions: $e', error: e));
      }
    });
  }
}
